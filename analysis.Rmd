...
d <- raw %>% 
  ...
  filter(!is.na(result)) %>%
  mutate(is_wrong = predicted(adjpoll_clinton, adjpoll_trump, adjpoll_johnson) != result)
d["error"] <- apply(
  d[c("adjpoll_clinton", "adjpoll_trump", "adjpoll_johnson")] - 
...
```{r}
cov_names <- c("adjpoll_clinton", "adjpoll_trump")
cov_names2 <- c("rawpoll_clinton", "rawpoll_trump")
mod <- prcomp(d[cov_names], scale. = T)
components <- mod$x
```
...
d %>% 
  ...
  ggplot(aes(adjpoll_clinton, gradeValue, color=result)) +
  scale_color_manual(values = pres_colors) +
  geom_point()
```
...
d %>% 
  filter(!is.na(gradeValue)) %>%
  ggplot(aes(gradeValue, error)) + 
  geom_point() +
  geom_smooth(method = "lm")
```

So grade value doesn't predict the prediction error either...
```{r}
summary(lm(error ~ gradeValue, data = d))
```
 I mean, *technically*, but it's not a large effect.
