...
d %>%
  ...
  ggplot(aes(is_wrong, grade_value)) +
  geom_boxplot()
```
...
table(USpolls$predicted, !USpolls$is_wrong) / nrow(USpolls)
state_polls <- d %>% filter(state != "U.S.")
table(state_polls$predicted, !state_polls$is_wrong) / nrow(state_polls)
```

```{r}
form1 <- is_wrong ~ end_date_to_elec + duration + samplesize + grade_value + is_national
form2 <- error ~ end_date_to_elec + duration + samplesize + grade_value + is_national
```
