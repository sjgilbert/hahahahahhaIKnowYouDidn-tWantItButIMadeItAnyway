...
These are the formulae that we're using for later model fitting and analyses.
```{r}
form1 <- is_correct ~ end_date_to_election + duration + samplesize + grade_value + is_national
form2 <- error ~ end_date_to_election + duration + samplesize + grade_value + is_national
```

```{r}
summary(mod1 <- lm(form1, d))
summary(mod2 <- lm(form1, d))
```
```{r}
d %>%
  filter(!is_national) %>%
  filter(state != "District of Columbia") %>%
  ggplot(aes(is_correct, error)) + 
  geom_boxplot()
```
