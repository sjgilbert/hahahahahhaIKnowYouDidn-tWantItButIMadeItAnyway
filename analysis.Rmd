...
make_state_results_data_frame <- function() {
  ...
  } clinton <- apply(state_results_raw2[c("Clinton", "H. Clinton", "Hillary Clinton")], 1, sum) trump <- apply(state_results_raw2[c("Trump", "D. Trump", "Donald Trump")], 1, sum) johnson <- apply(state_results_raw2[c("G. Johnson", "Gary Johnson")], 1, sum) other <- apply(state_results_raw2[-1], 1, sum) - clinton - trump - johnson  total_votes <- sum(county_raw$votes) votes_clinton <- sum(subset(county_raw, candidate %in% c("Clinton", "H. Clinton", "Hillary Clinton"))$votes) votes_trump <- sum(subset(county_raw, candidate %in% c("Trump", "D. Trump", "Donald Trump"))$votes)
...
d["end_date_to_election"] <- as.numeric(as.Date("11/09/2016", "%m/%d/%Y") - as.Date(d[["enddate"]], "%m/%d/%Y")) 
```
  ...
  left_join(d %>%
              ...
              summarize(predicted_clinton = sum(poll_clinton) / total_samplesize[1],
                        ...
                        error = (abs(predicted_clinton - actual_clinton[1]) + 
                                 abs(predicted_trump - actual_trump[1])) / 2),
            by=c("region" = "state"))
ggplot() +
